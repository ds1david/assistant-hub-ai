...
- [X] T010 [US2] Introduzir a flag local `resolved_at_least_once: bool` (inicia `False`) no laço de `capture_channel`, setada para `True` no mesmo ponto onde `reconnect_delay`/`woke_on_arrival` já são resetados após um `_capture_once` bem-sucedido — `agents/windows-audio-agent/src/assistant_hub_audio/capture.py` (depends on T002) (FR-004)

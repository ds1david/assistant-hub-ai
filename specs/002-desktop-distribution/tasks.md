...
- [ ] Criar tela de diagnóstico. (parcial: painéis agent/session/providers no shell; falta diagnóstico mic/GPU/deps unificado)
- [ ] Implementar armazenamento seguro de credenciais. (ainda `env`/`secretRef`; sem DPAPI/keyring OS)
...
- [ ] Criar workflow de release Windows. (parcial: `desktop-shell-smoke` no CI; falta pipeline de artefatos assináveis)
- [ ] Adicionar checksums e preparação para assinatura.
- [ ] Testar instalação, upgrade, rollback e remoção. (014 T033/T037 — validação GUI Windows residual)

...
from .transcriber import ModelManager, StreamingTranscriber, transcribe_file
...
async def health() -> dict:
    return {
        "status": "ok",
        "modelLoaded": model_manager.loaded,
...
async def load_model() -> dict:
    ...
    await asyncio.to_thread(model_manager.get)
    return {
        "status": "loaded",
        "model": settings.whisper_model,
        "device": settings.whisper_device,
...
async def audio_stream(websocket: WebSocket, session_id: str, channel_id: str) -> None:
    ...
    transcriber = StreamingTranscriber(settings, model_manager)
...
async def transcribe_uploaded_file(
    ...
    try:
        return await asyncio.to_thread(transcribe_file, temp_path, settings, model_manager, language)
    finally:

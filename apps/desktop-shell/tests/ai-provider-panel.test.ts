...
describe("renderAiProviderPanel", () => {
  ...
  it("onSave with pasted key sets os: secretRef and passes secret value (not kept in form)", () => {
    const container = document.createElement("div");
    document.body.appendChild(container);
    const onSave = vi.fn();
    ...
    const form = container.querySelector<HTMLFormElement>('[data-testid="ai-provider-form"]')!;
    (form.elements.namedItem("id") as HTMLInputElement).value = "p-os";
    (form.elements.namedItem("label") as HTMLInputElement).value = "OS";
    (form.elements.namedItem("baseUrl") as HTMLInputElement).value = "http://x.invalid";
    (form.elements.namedItem("model") as HTMLInputElement).value = "m";
    (form.elements.namedItem("authMode") as HTMLSelectElement).value = "bearer";
    (form.elements.namedItem("secretValue") as HTMLInputElement).value = "sk-super-secret-key-xyz";
    form.dispatchEvent(new Event("submit", { cancelable: true, bubbles: true }));

    expect(onSave).toHaveBeenCalledOnce();
    const saved = onSave.mock.calls[0][0] as Provider;
    const secretArg = onSave.mock.calls[0][1] as string;
    expect(saved.authentication.secretRef).toBe("os:assistant-hub/providers/p-os");
    expect(secretArg).toBe("sk-super-secret-key-xyz");
    // password field must not be serialized into provider object
    expect(JSON.stringify(saved)).not.toContain("sk-super-secret-key-xyz");
    document.body.removeChild(container);
  });
